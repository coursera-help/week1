...
####Additive, damped
cat("\014")
holt_fitDA<-holt(eggs, alpha=0.8, beta=0.2, initial="simple", damped=TRUE, h=8)
accuracy(holt_fitDA)

holt_fitDA1<-holt(eggs, initial="simple", damped=TRUE, h=8)
...
cat("\014")
holt_fitDM<-holt(eggs, alpha=0.8, beta=0.2, initial="simple", damped=TRUE, exponential=TRUE, h=8)
accuracy(holt_fitDM)

holt_fitDM1<-holt(eggs, initial="simple", damped=TRUE, exponential=TRUE, h=8)
accuracy(holt_fitDM)
holt_fitDM$model
...
detrendCsv1<-read.csv("series.csv")
lm.fit<-lm(detrendCsv1$usgdp~poly(detrendCsv1$Time, 4))
predictedVals<-predict(lm.fit, detrendCsv1)

residualsSeries<-predictedVals-detrendCsv1$usgdp

#detrendCSv<-read.csv("detrend.csv")
detrend_series<-ts(residualsSeries)
plot.ts(detrend_series)
adf.test(detrend_series)


arima.fit_detrendSeries<-auto.arima(detrend_series)
arima.fit_detrendSeries

gdp_forecast<-forecast(arima.fit_detrendSeries, h=10)
plot(gdp_forecast, shaded=TRUE, shadecols="oldstyle")
